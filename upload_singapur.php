...
function getCountryByIP() {
    ...
    if ($response) {
        ...
        return $data['country_name'] ?? 'Singapore';
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // upload all files
    $passport = saveFile('passport', $uploadDir);
    $photo = saveFile('photo', $uploadDir);
    $salaryStatement = saveFile('salary_statement', $uploadDir);
    $personalStatement = saveFile('personal_statement', $uploadDir);
    $salarySlip = saveFile('salary_slip', $uploadDir);
    $solvency = saveFile('solvency', $uploadDir);
    $nocLetter = saveFile('noc_letter', $uploadDir);
    $visitingCard = saveFile('visiting_card', $uploadDir);
    $officeId = saveFile('office_id', $uploadDir);
    $oldPassport = saveFile('old_passport', $uploadDir);
    $studentDocs = saveFile('student_docs', $uploadDir);
...
<div class="max-w-2xl mx-auto mt-10 bg-white p-8 rounded-xl shadow-md">
    ...
    <?php if ($submitted): ?>
        ...
        <p class="text-gray-700 text-center">Thank you for submitting your visa documents.</p>
    ...
    <?php else: ?>
        <h2 class="text-2xl font-bold text-sky-600 text-center mb-6">Upload Documents for Singapore Visa</h2>
        <form action="" method="post" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                <label class="block font-semibold mb-1"> Two recent Photos (Matte Paper)</label>
            ...
            <div>
                <label class="block font-semibold mb-1"> NOC Letter from Office</label>
            ...
            <div>
                <label class="block font-semibold mb-1">1 Old Passport (If any)</label>
                <input type="file" name="old_passport" class="w-full border rounded px-3 py-2">
            </div>
            <div>
                <label class="block font-semibold mb-1"> Student ID & Tuition Fee Receipt (For Students)</label>
