...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($admin_id) {
        ...
        if ($stmt->affected_rows > 0) {
            ...
            echo "<script>alert('Booking successful! You will be redirected to to your profile where you can see the bookings'); window.location.href='user_profile.php';</script>";
        } else {
            $stmt->close();
    ...
    <div class="max-w-xl mx-auto mt-10 p-6 bg-white rounded shadow">
        ...
        <form method="POST">
            <div class="mb-4">
                ...
                <input type="date" name="arrival_date" required class="w-full border px-3 py-2 rounded">
            </div>

            <div class="mb-4">
                <label class="block mb-1 font-semibold">Check-Out Date:</label>
                <input type="date" name="check_out" required class="w-full border px-3 py-2 rounded">
            </div>
